(function() {
  ...
  #
  #	Major change is that it uses an object {x,y,z} for positions instead of an array.
  #
  */  var OctNode, Octree;
  OctNode = (function() {
    function OctNode(position, size, data) {
  ...
  Octree = (function() {
    ...
    Octree.prototype.insertNode = function(root, size, parent, objData) {
      ...
      if (root === null) {
        pos = parent.position;
        offset = size / 2;
        branch = this.findBranch(parent, objData.position);
        newCenter = {
          x: 0,
          y: 0,
          z: 0
        ...
        branch = this.findBranch(root, objData.position);
        newSize = root.size / 2;
        ...
        if (root.data.length < this.MAX_OBJECTS) {
          ...
          for (_i = 0, _len = objList.length; _i < _len; _i++) {
            ob = objList[_i];
            _fn(ob);
            branch = this.findBranch(root, ob.position);
  ...
  # myTree = new Octree( 1500 )

  # num_tests = 20000
  # num_cols = 20000

  # rx = () -> (Math.random() * 900) - 450
  # p = (x,y,z) -> { x: x, y: y, z: z }
  ...
  # 	name = "Node_" + x
  # 	pos = p(rx(),rx(),rx())
  # 	testOb = new TO( name, pos )
  # 	myTree.insertNode( myTree.root, 1500, myTree.root, testOb )
  # console.profileEnd()
